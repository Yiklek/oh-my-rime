...
local function get_switches_list(key, env)
  ...
  for i = 1, size do
    table.insert(
      switches_list,
      { config:get_string("switches/@" .. i - 1 .. "/name"), config:get_string("switches/@" .. i .. "/reset") }
    )
  end
...
local function IsExistChar(obj, chars)
  if type(obj) ~= "table" or chars == "" then
    return ""
  end
  for i = 1, #obj do
    if obj[i][2] == chars then
      return obj[i][1]
    end
  end
  return ""
...
local function get_switch_keywords(obj, chars)
  ...
  for i = 1, #obj do
    if obj[i][1] == chars or obj[i][2] == chars then
      return obj[i][3]
    end
  end
  return ""
    ...
    if candidate_count then
      ...
      then -- 关键字切换方案选项开关，如简繁切换、拆分开关等等
        ...
        if keyword ~= "" then
          ...
          if flag ~= nil then
            if env.engine.context:get_option(keyword) then
              apply_switch(env, keyword, false)
            else
              apply_switch(env, keyword, true)
            end
            context:clear()
            return kAccepted
          end
