  ...
  if file then
    ...
    for line in file:lines() do
      ...
      if m and n then
        ...
        if name ~= "" then
          table.insert(schema_list, { n, name })
        end
      end
    end
    file:close()
...
local function splitCharPart(str)
  ...
  if str == nil or str == "" then
    ...
    if part[1] then
      ...
      if part[2] then
        part[2] = part[2]:gsub("\\r", "\r")
      end
      if part[3] then
        part[3] = part[3]:gsub("\\r", "\r")
      end
    end
  end
...
local function lunar_translator(input, seg)
  ...
  if input == keyword then
    local lunar = {
      {
        lunarDate.Date2LunarDate(os.date("%Y%m%d")) .. lunarJq.JQtest(os.date("%Y%m%d")),
        "〔公历⇉农历〕",
      },
...
local function QueryLunarInfo(date)
  ...
  if string.match(str, "^(20)%d%d+$") ~= nil or string.match(str, "^(19)%d%d+$") ~= nil then
    ...
    LunarDate = lunarDate.Date2LunarDate(str)
    LunarGz = lunarGz.lunarJzl(str)
    DateTime = lunarDate.LunarDate2Date(str, 0)
    if LunarGz ~= nil then
      result = {
...
local function number_translator(input, seg)
  ...
  if string.match(input, "^(%u+%d+)(%.?)(%d*)$") ~= nil then
    str = string.gsub(input, "^(%a+)", "")
    numberPart = number.number_translatorFunc(str)
