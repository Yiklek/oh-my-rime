...
local function commit_text_processor(key, env)
  ...
  if input_text:find("^%`*(%l+%`%l+)") then
    ...
    if key.keycode == 0x20 or key.keycode > 0x30 and key.keycode < 0x39 then
      if env.userphrase ~= "" and env.userphrase ~= nil and Wubi98Context.userphrasepath ~= "" then
        -- engine:commit_text(env.userphrase..env.inputtext.."\r")
        Wubi98Context.fileappendtext(Wubi98Context.userphrasepath, env.userphrase, env.inputtext, schema_name)
        env.userphrase = ""
        env.inputtext = ""
      end
    end
...
-- 记录自造词、文件路径Wubi98Context.userphrasepath在rime.lua中定义
Wubi98Context.fileappendtext = function(filepath, context, input, schemaname)
  lfmt("%s %s", input, filepath)
  if not context:find("%a") then
    input = Wubi98Context.splitinput(input, utf8.len(context))
...
-- 格式化五笔组合编码
Wubi98Context.splitinput = function(input, len)
  input = "`" .. input:gsub("%`*$", "")
